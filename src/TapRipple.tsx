...
import { Sparkles, Volume2, VolumeX, Trophy, Heart, Timer, Music } from 'lucide-react';
...
const TapRipple: React.FC = () => {
  ...
  const scales: Record<ScaleType, number[]> = {
    ...
    pentatonic: [261.63, 293.66, 329.63, 392.00, 440.00, 523.25, 587.33, 659.25], // Pentatonic
    blues: [261.63, 311.13, 349.23, 370.00, 392.00, 466.16, 523.25], // Blues
    japanese: [261.63, 277.18, 329.63, 349.23, 415.30, 523.25] // Japanese Scale
  };
  ...
  // 和音生成関数
  const createChord = useCallback((notes: number[]) => {
    ...
    notes.forEach((note, index) => {
      ...
      gameObjects.current.activeOscillators.push({ oscillator, gainNode, startTime: now });
    });
  }, [soundEnabled]);
  ...
  // 波紋の作成（長押し対応）
  const createRipple = (x: number, y: number, size: number = 1) => {
    const currentScaleNotes = scales[currentScale];
    const noteIndex = Math.floor((y / canvasRef.current!.height) * currentScaleNotes.length);
  ...
  const checkIntersections = () => {
    ...
    for (let i = 0; i < ripples.length; i++) {
      ...
      if (intersectingRipples.length > 1) {
        const key = intersectingRipples.map(r => r.id).sort().join('-');
        ...
        if (!gameObjects.current.intersections.has(key)) {
          ...
          if (uniqueNotes.length > 1) {
            ...
            // 交差点でパーティクル生成
            const centerX = intersectingRipples.reduce((sum, r) => sum + r.x, 0) / intersectingRipples.length;
            const centerY = intersectingRipples.reduce((sum, r) => sum + r.y, 0) / intersectingRipples.length;
  ...
  const gameLoop = useCallback((timestamp: number) => {
    ...
    if (showGrid) {
      ...
      for (let i = 0; i < scaleNotes.length; i++) {
        ...
        const noteIndex = Math.round(12 * Math.log2(scaleNotes[scaleNotes.length - 1 - i] / 261.63)) % 12;
    ...
    // 波紋の更新と描画
    gameObjects.current.ripples = gameObjects.current.ripples.filter(ripple => {
  ...
  const handleKeyDown = (e: KeyboardEvent) => {
    ...
    if (e.key === 'ArrowLeft') {
      const newIndex = (currentIndex - 1 + scaleNames.length) % scaleNames.length;
          ...
          {gameState === 'ready' && (
            <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/80 rounded-2xl">
              ...
              <p className="text-white mb-4 text-center px-8">
                画面をタップして美しい波紋を作ろう<br/>
                長押しまたは連続タップで大きな波紋<br/>
                波紋の重なりで和音を奏でる
              </p>
        ...
        {gameState === 'playing' && (
          <div className="mt-4 text-white text-center">
            <p className="text-sm opacity-80">
              {gameMode === 'zen' && '長押し or 連続タップで大きな波紋、重なりで和音を楽しもう'}
              {gameMode === 'score' && '緑のターゲットを狙って、和音で高得点を目指そう'}
